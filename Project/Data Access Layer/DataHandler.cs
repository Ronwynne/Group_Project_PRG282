...
namespace Project
{
    class DataHandler
    {
        ...
        public static List<Student> returnAllStudents()
        {
            ...
            while (reader.Read())
            {
                object[] arr = { reader[0], reader[1], reader[2], reader[3], reader[4], reader[5], reader[6], reader[7]};
                rows.Add(arr);
            }
            conn.Close();
            ...
            {
                temp.Add(new Student(int.Parse(row[0].ToString()), row[1].ToString(), row[2].ToString(), row[3].ToString(), 
                                    DateTime.Parse(row[4].ToString()), row[5].ToString(), row[6].ToString(), (byte[])row[7]));
            }
